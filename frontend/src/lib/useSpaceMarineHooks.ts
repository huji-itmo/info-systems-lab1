...
export const useCoordinates = (
  page = 0,
  size = 20,
  options?: UseQueryOptions<PaginatedResponse<Coordinates>, AxiosError>,
) =>
  useQuery<PaginatedResponse<Coordinates>, AxiosError>({
    queryKey: ['coordinates', { page, size }],
    queryFn: () =>
      apiClient
        .get('/coordinates', { params: { page, size } })
        .then((res) => res.data),
    ...options,
  });
...
export const useCoordinate = (
  id: number,
  options?: UseQueryOptions<Coordinates, AxiosError>,
) =>
  useQuery<Coordinates, AxiosError>({
    queryKey: ['coordinate', id],
    queryFn: () => apiClient.get(`/coordinates/${id}`).then((res) => res.data),
    enabled: !!id,
    ...options,
  });
...
export const useChapters = (
  page = 0,
  size = 20,
  options?: UseQueryOptions<PaginatedResponse<Chapter>, AxiosError>,
) =>
  useQuery<PaginatedResponse<Chapter>, AxiosError>({
    queryKey: ['chapters', { page, size }],
    queryFn: () =>
      apiClient
        .get('/chapters', { params: { page, size } })
        .then((res) => res.data),
    ...options,
  });
...
export const useChapter = (
  id: number,
  options?: UseQueryOptions<Chapter, AxiosError>,
) =>
  useQuery<Chapter, AxiosError>({
    queryKey: ['chapter', id],
    queryFn: () => apiClient.get(`/chapters/${id}`).then((res) => res.data),
    enabled: !!id,
    ...options,
  });
